 ...
 *
 * Source: buildscripts/idl/idlc.py --include src --base_dir build/opt --target_arch x86_64 --header build/opt/mongo/s/commands/cluster_multicast_gen.h --output build/opt/mongo/s/commands/cluster_multicast_gen.cpp src/mongo/s/commands/cluster_multicast.idl
 */
...
ClusterMulticastArgs::ClusterMulticastArgs(mongo::BSONObj multicast, std::string db) : _multicast(std::move(multicast)), _db(std::move(db)), _hasMulticast(true), _hasDb(true) {
    // Used for initialization only
}
