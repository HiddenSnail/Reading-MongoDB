...
public:
    ...
    CreateIndexesCmd();
    CreateIndexesCmd(std::string createIndexes, std::vector<mongo::NewIndexSpec> indexes);
    ...
    void setCreateIndexes(StringData value) & { auto _tmpValue = value.toString();  _createIndexes = std::move(_tmpValue); _hasCreateIndexes = true; }

    const std::vector<mongo::NewIndexSpec>& getIndexes() const& { return _indexes; }
    void getIndexes() && = delete;
    void setIndexes(std::vector<mongo::NewIndexSpec> value) & {  _indexes = std::move(value); _hasIndexes = true; }
...
private:
    ...
    std::vector<mongo::NewIndexSpec> _indexes;
